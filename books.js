var g_api_key = config.g_api_key;
var nyt_key = config.nyt_key;
fetch(
  "https://api.nytimes.com/svc/books/v3/lists.json?list-name=hardcover-fiction&api-key=" + nyt_key,
  {
    method: "get"
  }
)
...
function updateCover(id, isbn) {
  fetch(
    ...
    .catch(error => {
      console.log("Googel API Error: Defaulting to archival images.");
      $.each(archivedImages, function(index, url) {
        var id = index + 1;
        var img = url;
        $("#cover-" + id).attr("src", img);
      });
